...
> **Change these values for each project.** Everything below this section is shared development standards.

- **Project**: OnionShield — an OSINT map platform with military/geopolitical data layers, K8s-based infrastructure
- **Server SSH**: `ssh ai@192.168.8.8` (K8s cluster host)
- **Kubectl**: `kubectl XXX` (direnv auto-sets `KUBECONFIG` to the microk8s-ai context when you `cd` into this project — no `--context` flag needed. See `.envrc` in project root.)
- **Domain (production)**: `0mg.ai`
...
For local development run `./dev.sh`.

---

## Playwright Authentication (Deployed Services Only)

**Use Bearer token auth when testing deployed production services:**

```javascript
// 1. Read CLAUDE_SERVICE_TOKEN from token file
// 2. Set header BEFORE navigation
await page.setExtraHTTPHeaders({
  'Authorization': 'Bearer <token>'
});
// 3. Navigate
await page.goto('https://www.kellytrade.org');
...
## Active Technologies
- Python 3.13 (backend — simulation service), TypeScript 5.9.3 (frontend) + Existing — FastAPI, pydantic (backend); React 19, react-map-gl, maplibre-gl (frontend) (010-ukraine-scenario-realism)
- N/A — scenario JSON file with embedded territorial polygons (010-ukraine-scenario-realism)
- Python 3.13 (preprocessing pipeline + backend service) + shapely (NEW — polygon merge/dissolve), existing FastAPI/pydantic (011-viina-real-data-migration)
- File-based cache (compressed JSON per day) in `geodata/cache/` (011-viina-real-data-migration)

## Recent Changes
- 010-ukraine-scenario-realism: Added Python 3.13 (backend — simulation service), TypeScript 5.9.3 (frontend) + Existing — FastAPI, pydantic (backend); React 19, react-map-gl, maplibre-gl (frontend)
