...
def build_turns(messages: List[Dict[str, Any]]) -> List[Turn]:
    ...
    def flush_turn():
        nonlocal current_user, assistant_order, assistant_latest, tool_results_by_id, tool_uses_by_id, turns
        if current_user is None:
            return
        if not assistant_latest:
            return
        assistants = [assistant_latest[mid] for mid in assistant_order if mid in assistant_latest]
        turns.append(Turn(user_msg=current_user, assistant_msgs=assistants, tool_results_by_id=dict(tool_results_by_id), tool_uses_by_id=dict(tool_uses_by_id)))
...
# ----------------- Langfuse emit -----------------
def _tool_calls_from_accumulated(tool_uses_by_id: Dict[str, Dict[str, Any]]) -> List[Dict[str, Any]]:
    """Build tool call list from accumulated tool_use blocks (not from deduplicated assistant msgs)."""
    calls: List[Dict[str, Any]] = []
    for tid, tu in tool_uses_by_id.items():
        calls.append({
            "id": tid,
...
def emit_turn(langfuse: Langfuse, trace_id: str, session_id: str, turn_num: int, turn: Turn, transcript_path: Path) -> None:
    ...
    tool_calls = _tool_calls_from_accumulated(turn.tool_uses_by_id)
